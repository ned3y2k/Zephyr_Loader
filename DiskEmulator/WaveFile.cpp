...
//-----------------------------------------------------------------------------
HRESULT CWaveFile::Open(LPSTR strFileName, WAVEFORMATEX* pwfx, SOUND_FILE_MODE mode )
{
    HRESULT hr;
...
HRESULT CWaveFile::OpenFromMemory( BYTE* pbData, ULONG ulDataSize, 
                                   WAVEFORMATEX* pwfx, SOUND_FILE_MODE mode )
{
    m_pWaveformatex       = pwfx;
...
HRESULT CWaveFile::ReadMMIO()
{
    ...
    {
        m_pWaveformatex = (WAVEFORMATEX*)new CHAR[ sizeof(WAVEFORMATEX) ];
        if( NULL == m_pWaveformatex )
    ...
    else
    {
        ...
        m_pWaveformatex = (WAVEFORMATEX*)new CHAR[ sizeof(WAVEFORMATEX) + cbExtraBytes ];
        if( NULL == m_pWaveformatex )
            return DXTRACE_ERR( TEXT("new"), E_FAIL );

        // Copy the bytes from the pcm structure to the waveformatex structure
        memcpy( m_pWaveformatex, &pcmWaveFormat, sizeof(pcmWaveFormat) );
        m_pWaveformatex->cbSize = cbExtraBytes;

        // Now, read those extra bytes into the structure, if cbExtraAlloc != 0.
        if( mmioRead( m_hmmio, (CHAR*)(((BYTE*)&(m_pWaveformatex->cbSize))+sizeof(WORD)),
                      cbExtraBytes ) != cbExtraBytes )
        {
            SAFE_DELETE( m_pWaveformatex );
            return DXTRACE_ERR( TEXT("mmioRead"), E_FAIL );
        }
    }
...
//-----------------------------------------------------------------------------
DWORD CWaveFile::GetTotalSamples()
{
    return m_dwSize / m_pWaveformatex->nBlockAlign;
}
...
{
	DWORD dwBeginBytes = dwBeginSamples*m_pWaveformatex->nBlockAlign;
